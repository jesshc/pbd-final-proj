...
val hdfsInputPath = "hdfs:///user/jhc10024_nyu_edu/hw6/nypd_cfs_cleaned_1"
...
// Part 1d: Compare response times for same incident type across boroughs

println("=== Response Time by Borough x Incident Type ===")

val dfTypeFiltered = df
  .filter(col("TYP_DESC_ENUM").isNotNull)
  .filter(col("RESPONSE_TIME_SEC").isNotNull)
  .filter(col("RESPONSE_TIME_SEC") > 0)

val boroughTypeStats = dfTypeFiltered
  .groupBy("BORO_NM", "TYP_DESC_ENUM")
  .agg(
    avg("RESPONSE_TIME_SEC").alias("avg_response_sec"),
    expr("percentile_approx(RESPONSE_TIME_SEC, 0.5)").alias("median_response_sec"),
    count("*").alias("num_records")
  )
  .orderBy(col("TYP_DESC_ENUM"), desc("avg_response_sec"))

boroughTypeStats.show(50, false)
