...
val dfBoroughStats =
  dfFiltered
    ...
    .withColumn("STDDEV_RESPONSE_TIME", date_format(from_unixtime(col("STDDEV_RESPONSE_SEC")), "HH:mm:ss"))
...
val dfPrecinctStats =
  dfFiltered
    ...
    .withColumn(
      ...
      date_format(from_unixtime(col("STDDEV_RESPONSE_SEC")), "HH:mm:ss")
    )
    .orderBy(desc("AVG_RESPONSE_SEC"))
    .drop(
      "AVG_RESPONSE_SEC",
...
dfPrecinctStats.orderBy(desc("AVG_RESPONSE_SEC")).show(10, false)

// Top 10 fastest precincts
println("=== 3b. Top 10 Fastest Precincts ===");
dfPrecinctStats.orderBy(asc("AVG_RESPONSE_SEC")).show(10, false)
...
dfPrecinctResponseRanked.show(200, false)


// Save all generated dataframes to HDFS for visualization usage later
val hdfsOutputPath = "hdfs:///user/jhc10024_nyu_edu/FinalProject/AnalysisOutputs/Part1"

dfBoroughStats.write.mode("overwrite").parquet(hdfsOutputPath + "/dfBoroughStats")
dfBoroughResponse.write.mode("overwrite").parquet(hdfsOutputPath + "/dfBoroughResponse")
dfPrecinctStats.write.mode("overwrite").parquet(hdfsOutputPath + "/dfPrecinctStats")
dfPrecinctResponseRanked.write.mode("overwrite").parquet(hdfsOutputPath + "/dfPrecinctResponseRanked")