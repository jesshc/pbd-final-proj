...
// 1. Load CSV
// =========================
val df = spark.read.option("header", "true").option("inferSchema", "false").option("mode", "PERMISSIVE").csv("./hw6/nypd_cfs.csv")
...
// Map records to 1 and count using RDD
...
// =========================
// 4. RADIO_CODE vs TYP_DESC discrepancies
// =========================
...
// Group by RADIO_CODE to see how many TYP_DESC values map to each code
...
// Show RADIO_CODEs that map to multiple TYP_DESC (inconsistencies)
println("RADIO_CODE values mapping to multiple TYP_DESC:")
codeToDesc.filter(col("num_descs") > 1).show(false)


// =========================
// 5. Validate consistency between TYP_DESC and CIP_JOBS
// =========================
// This checks whether each incident description (TYP_DESC) is consistently
// associated with a single CIP_JOBS category.
...
// 8. Max string length of TYP_DESC
...
// 9. Check if INCIDENT Timestamp is before ADD_TS
val dfWithTimes = df
  .withColumn(
    "INCIDENT_TS",
    to_timestamp(
...
// Count rows where incident happens AFTER add time (invalid)
val invalidCount = dfWithTimes.filter(
  col("INCIDENT_TS").isNotNull &&
    col("ADD_TS_TS").isNotNull &&
    col("INCIDENT_TS") > col("ADD_TS_TS")
).count()

println(s"Invalid rows (incident after add): $invalidCount")


// 10. Check for out-of-range time

val anomalies = df.select(
  col("CAD_EVNT_ID"),
  // 10.1 INCIDENT_TIME (24-hour format)
...
// 11. Check Dispatch Timestamp is after Added Timestamp
val invalidCount = dfWithTS.filter(
  col("DISP_TS_TS") < col("ADD_TS_TS")
).count()

println(s"DISP before ADD count: $invalidCount")